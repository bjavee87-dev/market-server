import { CryptoTrades } from "../model";

const create = async (data) => {
	var oldData = await CryptoTrades.findOne({ date: data.date, symbol: data?.symbol })
	if (!oldData) {
		const newData = new CryptoTrades({
			...data
		})
		await newData.save();
		return true
	} else {
		await CryptoTrades.updateOne(
...
const findOne = async (filter: any) => {
	...
	return await CryptoTrades.findOne(newFilter);
}
const find = async (filter: any) => {
	const newFilter = { ...filter };
	return await CryptoTrades.find(newFilter);
}
const update = async (filter: any, newData: any) => {
	const updateData = { ...newData };
	const newFilter = { ...filter };
	return await CryptoTrades.updateOne(
		newFilter,
		{ $set: updateData }
	);
}
const remove = async (filter: any) => {
	const newFilter = { ...filter };
	return await CryptoTrades.findOneAndDelete(newFilter);
}
